...
for n in range(len(mymods)):
    ...
    print( m )
    if m != 'NM2DMT':
        if not exists( "nnl_mm_outlierness_"+m+"_x2.csv" ):
...
if alldf.shape[0] > 0:
    alldf.to_csv( "nnl_mm_outlierness_x2.csv", index=False )
...
if alldf.shape[0] > 0:
    alldf.to_csv( "nnl_mm_outlierness_x2_filt.csv", index=False )

import antspymm
import pandas as pd
alldf=pd.read_csv("nnl_mm_outlierness_x2_filt.csv")
